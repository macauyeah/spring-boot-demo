...
ARG git.build.version=default-docker-build
RUN --mount=type=cache,target=/root/.m2/repository/ mvn clean compile package -P ${mavenProfile} -Dgit.build.version=${git.build.version} -Dmaven.test.skip=true
